...
const AdminDashboard = () => {
  ...
  useEffect(() => {
    // Only run if you have a token (optional)
    const storedToken = localStorage.getItem('token');
  
    // Fetch past notifications
    axios.get(`${import.meta.env.VITE_API_URL || 'http://localhost:5000'}/api/admin/notifications`, {
  ...
  // Map your simple notifications into the shape AdminNotifications expects
const mappedNotifications = notifications.map(n => ({
  id: n.id,
  title: n.message,           // using the message as the title
  description: n.message,     // or split it out however you like
  createdAt: n.createdAt,
  read: false,                // default unread
  type: "car",                // choose or derive a type
  priority: "normal"          // choose or derive a priority
}));
  ...
  {notifications.length}
          ...
          >
            ...
            {/* Notifications Tab */}
              {activeTab === "notifications" && (
                <AdminNotifications notifications={mappedNotifications} />
              )}
